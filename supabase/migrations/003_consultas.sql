-- ###########################################################################
-- CONSULTAS RUC / DNI / TIPO DE CAMBIO  ·  @rodatech/consultas
-- ###########################################################################
--
-- Estas tablas y funciones NO las define el esquema: las define el paquete
-- @rodatech/consultas, cuyo código ya llama a estos nombres de RPC. El
-- contrato lo fija el llamador.
--
-- Origen: packages/consultas/migracion.sql — si ese archivo cambia, este debe
-- regenerarse desde él. No editar aquí.
--
-- Va antes de las funciones y de RLS porque 006_rls.sql habilita row level
-- security recorriendo una lista de tablas que incluye estas tres.
-- ###########################################################################
...
-- =============================================================================
--
-- Este SQL NO está en supabase/migrations/ porque ese directorio lo administra
-- otro agente en paralelo. Alguien con acceso al proyecto de Supabase debe
-- copiar este archivo (o su contenido) a la siguiente migración numerada del
-- proyecto e integrarla al flujo normal de `pnpm db:aplicar`. Ver README.md.
--
-- Contenido:
