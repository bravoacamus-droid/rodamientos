...
export function Cobrador({
  ...
  const [resultado, cobrar, cobrando] = useActionState<ResultadoCobro | null, FormData>(
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-lg">
        ...
        <form action={cobrar} className="mt-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2">
            <label className="flex flex-col gap-1">
              ...
              {!salda ? (
                ...
                <span className="text-xs text-[var(--ok)]">Queda saldado.</span>
              )}
            </label>
                ...
                {reparto.map((r) => (
                  <li key={r.cuota.id} className="flex justify-between gap-2">
                    <span className="text-[var(--fg-muted)]">
                      Cuota {r.cuota.numero} · vence {r.cuota.fecha_vencimiento}
                    </span>
                    <span className="tabular">
                      ...
                      >
                        {r.quedaSaldo === 0 ? "saldada" : `quedan ${r.quedaSaldo.toFixed(2)}`}
                      </span>
                    </span>
                  </li>
                ))}
