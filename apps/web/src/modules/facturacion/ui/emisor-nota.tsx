...
export function EmisorNota({
  ...
  const [resultado, emitir, emitiendo] = useActionState<ResultadoNota | null, FormData>(
  ...
  const bloqueos = bloqueosNota(documento, tipo, motivo, monto, fecha, yaAcreditado);
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-lg">
        ...
        <form action={emitir} className="mt-4 flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-1">
            ...
            <SelectNativo value={motivo} onChange={(e) => setMotivo(e.target.value)}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1">
              <span className="text-sm font-medium">
                Importe con IGV <span className="text-[var(--danger)]">*</span>
              </span>
              <Input
                type="number"
                min={0}
            ...
            {monto > 0 && Math.abs(totalReal - monto) >= 0.01 ? (
              <div className="anim-entrada self-end rounded-sm border border-[var(--border)] bg-[var(--surface-2)] p-2 text-xs">
                Saldrá por <strong className="tabular">{totalReal.toFixed(2)}</strong>,
                no {monto.toFixed(2)}: el IGV se calcula sobre la base redondeada.
              </div>
            ) : null}
          </div>
          ...
          {!total && !esCorreccionSinImporte(motivo) ? (
            ...
            <p className="rounded-sm border border-[var(--border)] bg-[var(--surface-2)] p-2.5 text-xs">
              La nota copiará las{" "}
              <strong>{documento.lineas.length}</strong>{" "}
              {documento.lineas.length === 1 ? "línea" : "líneas"} del documento
              original: con este motivo se está corrigiendo la operación entera.
            </p>
          )}
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!motivo || bloqueos.length > 0 || emitiendo}>
